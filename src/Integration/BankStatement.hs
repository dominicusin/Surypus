...
module Integration.BankStatement
  ...
  , parseOFX
  , parseISO20022
  -- , importStatementLines  -- DISABLED: Opaleye dependency removed
  -- , matchTransactionsToBills  -- DISABLED: Opaleye dependency removed
  -- , flagUnmatchedTransactions  -- DISABLED: Opaleye dependency removed
  ) where
...
-- Hasql imports commented out - database operations disabled
...
-- DISABLED: Database operations require proper pool setup
...
matchTransactionsToBills pool importId = do
   ...
   let bankStatementLineTable = OE.table "bank_statement_line" (OITag.tag "bank_statement_line")
         \(importIdCol, txnDateCol, valueDateCol, amountCol, currencyCol, descriptionCol, refNumberCol, counterpartyCol, matchedBillIdCol, needsReviewCol) ->
           ( importIdCol
           , txnDateCol
           , valueDateCol
           , amountCol
         ...
         \(importIdCol, txnDateCol, valueDateCol, amountCol, currencyCol, descriptionCol, refNumberCol, counterpartyCol, matchedBillIdCol, needsReviewCol) ->
           ( OE.required importIdCol
           , OE.required txnDateCol
           , OE.required valueDateCol
           , OE.required amountCol
           , OE.required currencyCol
           , OE.required descriptionCol
           , OE.required refNumberCol
           , OE.required counterpartyCol
           , OE.required matchedBillIdCol
           , OE.required needsReviewCol
           )
   
   let billTable = OE.table "bill" (OITag.tag "bill")
         \(billIdCol, amountCol, dueDateCol, statusCol, paidDateCol) ->
           ( billIdCol
           , amountCol
           , dueDateCol
           , statusCol
         ...
         \(billIdCol, amountCol, dueDateCol, statusCol, paidDateCol) ->
           ( OE.required billIdCol
           , OE.required amountCol
           , OE.required dueDateCol
           , OE.required statusCol
           , OE.required paidDateCol
           )
   ...
   let txnQuery = OE.sql 
         ...
         (OE.makeColumns (,,) 
            ...
            OE.double
            OE.text
         ) (OE.required . (\(importIdCol, _, _, _) -> importIdCol))
   txnResult <- runQuery pool txnQuery importId
   case txnResult of
     Left err -> return $ QueryError (T.pack $ show err)
     Right txns -> do
       -- Get unpaid bills
       let billsQuery = OE.sql 
       ...
       billsResult <- runQuery pool billsQuery
       case billsResult of
         Left err -> return $ QueryError (T.pack $ show err)
         Right bills -> do
           ...
           let updateMatchQuery = OE.sql 
                 "UPDATE bill SET status = 'paid', paid_date = $2::DATE WHERE id = $1::UUID"
                 (OE.makeColumns (,) 
                    OE.text
                    OE.text
                 ) OE.noCols
           mapM_ (\(billId, txnDate) -> runCommand pool updateMatchQuery (billId, txnDate)) matches
           
           -- Link transactions to bills
           let linkQuery = OE.sql 
                 "UPDATE bank_statement_line SET matched_bill_id = $1::UUID WHERE id = $2::UUID"
                 (OE.makeColumns (,) 
                    OE.text
                    OE.text
                 ) OE.noCols
           mapM_ (\(billId, txnId) -> runCommand pool linkQuery (billId, txnId)) matches
...
-- DISABLED: Opaleye dependency removed from cabal
