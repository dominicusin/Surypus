...
listJobs pool = do
  let sql =
      ...
      stmt = unpreparable sql E.noParams (D.rowList jobRowDecoder)
...
listPendingJobs pool = do
  let sql = "SELECT COUNT(*) FROM jobs WHERE job_status = 'pending'"
      stmt = unpreparable sql E.noParams (D.singleRow (D.column (D.nonNullable D.int8)))
  result <- use pool $ Session.statement () stmt
  case result of
...
processNextJob pool = do
  let selectSql =
      ...
      selectStmt = unpreparable selectSql E.noParams (D.rowMaybe jobRowDecoder)
