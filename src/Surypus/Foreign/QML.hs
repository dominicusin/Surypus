...
calculateBillTotals bills = 
  let total = sum (map billTotal bills)
      vat = sum (map billVatSum bills)
      totalWithVat = total + vat
  in (total, vat, totalWithVat)

-- | Calculate stock value
calculateStockValue :: [Stock] -> Double
calculateStockValue stock = fromIntegral (sum (map stockQuantity stock))
...
calculatePayrollTotals payroll =
  let accrued = sum (map payrollAccrued payroll)
      tax = sum (map payrollTaxNdfl payroll)
      social = sum (map payrollSocialContribution payroll)
      net = sum (map payrollNetPay payroll)
