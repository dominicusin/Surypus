...
verifyBillTotal expectedTotal lines =
    let actualTotal = sum [price * (1 - discount/100) | (price, discount) <- lines]
...
validateBill lines =
    let totals = map (\(p, q, t) -> p * q * (1 + t / 100)) lines
        total = sum totals
        priceCheck = all (\(p, _, _) -> p >= 0) lines
        qtyCheck = all (\(_, q, _) -> q > 0) lines
        taxCheck = all (\(_, _, t) -> t >= 0 && t <= 100) lines
