...
mkPerson id name kind inn = Person
    { pId = id
    , pCode = Nothing
    , pName = name
    , pINN = Just inn
...
mkGoods id name gtype unitId = Goods
    { gId = id
    , gCode = Nothing
    , gName = name
    , gBarcode = Nothing
...
mkLocation id name ltype = Location
    { lId = id
    , lCode = Nothing
    , lName = name
    , lLocationType = ltype
...
processSale bill lines
    | bStatus bill /= BS_Draft = Left (T.pack "Bill must be in draft status")
    | null lines = Left (T.pack "Bill must have at least one line")
    | otherwise = Right bill { bTotal = calcBillTotal lines }

-- | Post a bill (generate accounting entries)
postBill :: Bill -> [BillLine] -> [AccTurn]
postBill bill lines = concatMap (generateEntries bill) lines
