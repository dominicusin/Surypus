...
data JasperTextField = JasperTextField
  ...
  , jtfWidth :: Int
  , jtfHeight :: Int
  , jtfExpression :: Text  -- $F{field} or $V{variable} or formula
...
data JasperImage = JasperImage
  ...
  , jiWidth :: Int
  , jiHeight :: Int
  , jiExpression :: Text  -- $F{image} or URL
...
convertCrystalToJasper cr = JasperReport
  { jrName = crName cr
  , jrColumnCount = 1
...
convertObjectsToBand objs = JasperBand
  { jbHeight = 50  -- default
  , jbIsSplitAllowed = False
  , jbElements = map convertElement objs
  }

convertElement :: CrystalReportObject -> JasperElement
convertElement (TextObject cto) = JStaticText JasperStaticText
...
convertElement (FieldObject cfo) = JTextField JasperTextField
...
convertElement (FormulaObject cformula) = JTextField JasperTextField
...
convertElement (LineObject clo) = JLine JasperLine
...
convertElement (BoxObject cbo) = JRectangle JasperRectangle
...
convertElement (SubreportObject cs) = JSubreport JasperSubreport
  ...
  , jsParameters = map (\(a,b) -> (a, "$P{" <> b <> "}")) (csLinkFields cs)
  }
convertElement (GraphObject cgo) = JChart JasperChart
...
convertDatabaseFields = map convertField
  where
    convertField cdf = JasperField
      { jfName = cdfName cdf
...
convertFormulaFields = map convertFormula
  where
    convertFormula cff = JasperVariable
      { jvName = cffName cff
...
convertGroups = map convertGroup
  where
    convertGroup cg = JasperGroup
      { jgName = cgName cg
...
renderJasperXml jr = T.concat
  ...
  , "name=\"", jrName jr, "\" "
  , "columnCount=\"", T.pack (show (jrColumnCount jr)), "\" "
  , "printOrder=\"", jrPrintOrder jr, "\" "
  , "orientation=\"", jrOrientation jr, "\" "
  , "pageWidth=\"", T.pack (show (jrPageWidth jr)), "\" "
  , "pageHeight=\"", T.pack (show (jrPageHeight jr)), "\" "
  , "columnWidth=\"", T.pack (show (jrColumnWidth jr)), "\" "
  , "columnSpacing=\"", T.pack (show (jrColumnSpacing jr)), "\" "
  , "leftMargin=\"", T.pack (show (jrLeftMargin jr)), "\" "
  , "rightMargin=\"", T.pack (show (jrRightMargin jr)), "\" "
  , "topMargin=\"", T.pack (show (jrTopMargin jr)), "\" "
  , "bottomMargin=\"", T.pack (show (jrBottomMargin jr)), "\" "
  , "isTitleNewPage=\"", if jrIsTitleNewPage jr then "true" else "false", "\" "
  , "isSummaryNewPage=\"", if jrIsSummaryNewPage jr then "true" else "false", "\">\n"
...
renderFields fields = T.concat $ map renderField fields

renderField :: JasperField -> Text
renderField jf = T.concat
  [ "  <field name=\"", jfName jf, "\" class=\"", jfClass jf, "\"/>\n"
  ]

renderVariables :: [JasperVariable] -> Text
renderVariables vars = T.concat $ map renderVariable vars

renderVariable :: JasperVariable -> Text
renderVariable jv = T.concat
  [ "  <variable name=\"", jvName jv, "\" class=\"", jvClass jv, "\" "
  , "resetType=\"", jvResetType jv, "\" "
  , "calculation=\"", jvCalculation jv, "\">\n"
...
renderGroups groups = T.concat $ map renderGroup groups

renderGroup :: JasperGroup -> Text
renderGroup jg = T.concat
  [ "  <group name=\"", jgName jg, "\" "
  , "isStartNewColumn=\"", if jgIsStartNewColumn jg then "true" else "false", "\" "
  , "isStartNewPage=\"", if jgIsStartNewPage jg then "true" else "false", "\" "
  , "isResetPageNumber=\"", if jgIsResetPageNumber jg then "true" else "false", "\">\n"
  , "    <groupExpression><![CDATA[", jgExpression jg, "]]></groupExpression>\n"
...
renderBand jb = T.concat
  [ "  <band height=\"", T.pack (show (jbHeight jb)), "\" "
  , "isSplitAllowed=\"", if jbIsSplitAllowed jb then "true" else "false", "\">\n"
  , T.concat $ map renderElement (jbElements jb)
  , "  </band>\n"
  ]

renderElement :: JasperElement -> Text
renderElement (JStaticText jst) = T.concat
  [ "    <staticText>\n"
  , "      <reportElement x=\"", T.pack (show (jstX jst)), "\" "
  , "y=\"", T.pack (show (jstY jst)), "\" "
  , "width=\"", T.pack (show (jstWidth jst)), "\" "
  , "height=\"", T.pack (show (jstHeight jst)), "\"/>\n"
  , "      <text><![CDATA[", jstText jst, "]]></text>\n"
  , "    </staticText>\n"
  ]
renderElement (JTextField jtf) = T.concat
  [ "    <textField>\n"
  , "      <reportElement x=\"", T.pack (show (jtfX jtf)), "\" "
  , "y=\"", T.pack (show (jtfY jtf)), "\" "
  , "width=\"", T.pack (show (jtfWidth jtf)), "\" "
  , "height=\"", T.pack (show (jtfHeight jtf)), "\"/>\n"
  , "      <textFieldExpression><![CDATA[", jtfExpression jtf, "]]></textFieldExpression>\n"
  , "    </textField>\n"
  ]
renderElement (JLine jl) = T.concat
  [ "    <line>\n"
  , "      <reportElement x=\"", T.pack (show (jlX jl)), "\" "
  , "y=\"", T.pack (show (jlY jl)), "\" "
  , "width=\"", T.pack (show (jlWidth jl)), "\" "
  , "height=\"", T.pack (show (jlHeight jl)), "\"/>\n"
  , "    </line>\n"
  ]
renderElement (JRectangle jr) = T.concat
  [ "    <rectangle>\n"
  , "      <reportElement x=\"", T.pack (show (jrX jr)), "\" "
  , "y=\"", T.pack (show (jrY jr)), "\" "
  , "width=\"", T.pack (show (jrWidth jr)), "\" "
  , "height=\"", T.pack (show (jrHeight jr)), "\"/>\n"
  , "    </rectangle>\n"
  ]
renderElement (JImage ji) = T.concat
  [ "    <image>\n"
  , "      <reportElement x=\"", T.pack (show (jiX ji)), "\" "
  , "y=\"", T.pack (show (jiY ji)), "\" "
  , "width=\"", T.pack (show (jiWidth ji)), "\" "
  , "height=\"", T.pack (show (jiHeight ji)), "\"/>\n"
  , "      <imageExpression><![CDATA[", jiExpression ji, "]]></imageExpression>\n"
  , "    </image>\n"
  ]
renderElement (JSubreport js) = T.concat
  [ "    <subreport>\n"
  , "      <reportElement x=\"", T.pack (show (jsX js)), "\" "
  , "y=\"", T.pack (show (jsY js)), "\" "
  , "width=\"", T.pack (show (jsWidth js)), "\" "
  , "height=\"", T.pack (show (jsHeight js)), "\"/>\n"
  , "      <subreportExpression><![CDATA[", jsReportExpression js, "]]></subreportExpression>\n"
  , "    </subreport>\n"
  ]
renderElement (JChart jch) = T.concat
  [ "    <", jchChartType jch, ">\n"
  , "      <chart>\n"
  , "        <reportElement x=\"", T.pack (show (jchX jch)), "\" "
  , "y=\"", T.pack (show (jchY jch)), "\" "
  , "width=\"", T.pack (show (jchWidth jch)), "\" "
  , "height=\"", T.pack (show (jchHeight jch)), "\"/>\n"
  , "      </chart>\n"
  , "    </", jchChartType jch, ">\n"
  ]
