...
runServer cfg = do
  ...
  middleware
          ...
          Just user -> do
            ...
            case authResult of
              ...
              Just authUser -> do
                ...
                tokenConfig <- liftIO $ pure (jwtConfigFromSecret scJwtSecret)
      ...
      result <- liftIO $ runAppM appEnv $ do
        ...
        case maybeAppUser of
          ...
          Right appUser -> do
            ...
            tokenConfig <- pure (jwtConfigFromSecret scJwtSecret)
            accessToken <- generateAccessToken tokenConfig payload
            case accessToken of
              Left err -> throwE (InternalError err)
              Right token ->
    ...
    $ do
      ...
      result <- liftIO $ runAppM env $ do
        locationService <- getLocationService
        createLocation locationService input
      case result of
        Left appErr -> respondAppError appErr
    ...
    $ do
      ...
      result <- liftIO $ runAppM env $ do
        locationService <- getLocationService
        updateLocation locationService lid input
      case result of
        Left appErr -> respondAppError appErr
        Right location -> json $ object ["success" .= True, "data" .= location]
     delete
