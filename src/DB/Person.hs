...
listPersons pool (Pagination limit offset) PersonFilter{..} = use pool $
  Session.statement (limit, offset, pfName, pfINN, pfKind, pfStatus) stmt
  where
    stmt = Statement
...
getPerson pool pid = use pool $
  Session.statement pid stmt
  where
    stmt = Statement
...
createPerson pool Person{..} = use pool $
  Session.statement
    ( personCode
    , personName
    , personINN
...
updatePerson pool pid Person{..} = use pool $
  Session.statement
    ( pid
    , personCode
    , personName
