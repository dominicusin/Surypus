...
listGoods pool (Pagination limit offset) GoodsFilter{..} = use pool $
...
createGoods pool Goods{..} = use pool $
  Session.statement
    ( goodsCode
    , goodsName
    , goodsBarcode
...
updateGoods pool gid Goods{..} = use pool $
  Session.statement
    ( gid
    , goodsCode
    , goodsName
...
deleteGoods pool gid = use pool $
  Session.statement gid stmt *> pure True
  where
    stmt = Statement
