...
listTechs pool TechFilter{..} = use pool $
  Session.statement
    ( namePattern
    , tfGoodsId
    )
...
calculateTechTime pool tid = use pool $
  Session.statement tid stmt
  where
    stmt = Statement
...
calculateTechCost pool tid materialCost = use pool $
  Session.statement
    (tid, materialCost)
    stmt
  where
...
toMaybeDouble m = fmap toRealFloat m
