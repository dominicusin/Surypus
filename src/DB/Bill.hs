...
listBills pool (Pagination limit offset) BillFilter{..} = use pool $
...
createBill pool bill@Bill{..} = do
  newId <- use pool $ Session.statement
    ( billCode
    , billOpId
    , billDate
    , billPersonId
...
recalcBillTotals pool bid = use pool $
  Session.statement bid stmt *> pure ()
  where
    stmt = Statement
...
setEdiStatus pool bid status conf = use pool $
