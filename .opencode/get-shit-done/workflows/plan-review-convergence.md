...
```bash
INIT=$(node "/home/domini/src/My/Surypus/.opencode/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "/home/domini/src/My/Surypus/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "/home/domini/src/My/Surypus/.opencode/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
