-- V308__rbac_canonical_finalization_validation_test.sql
-- Validate that canonicalization produced no NULL canonical_path values.
DO $$
DECLARE
  rec RECORD;
  cnt INTEGER;
BEGIN
  -- Run the canonicalization if function exists
  ...
  FOR rec IN (
    ...
    IF cnt > 0 THEN
      RAISE EXCEPTION 'Canonicalization check failed: %.% has % NULL canonical_path(s)', rec.schema, rec.table, cnt;
    END IF;
  END LOOP;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'rbac' AND table_name = 'wrapper') THEN
    ...
    IF cnt > 0 THEN
      RAISE EXCEPTION 'RBAC wrapper canonicalization check failed: % NULL canonical_path(s)', cnt;
    END IF;
  END IF;
END;
$$ LANGUAGE plpgsql;
