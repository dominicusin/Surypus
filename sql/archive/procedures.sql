...
-- Additional unified guard wrapper (for testability and fallback)
...
BEGIN
  ...
  BEGIN
    v_ok := FALSE;
    v_ok := has_permission_compat(p_user_id::BIGINT, 'inventory_write', p_tenant_id);
...
-- Compatibility gate: inventory write across all cmd_inventory_* functions
CREATE OR REPLACE FUNCTION guard_inventory_write(
  p_user_id UUID,
  p_tenant_id UUID
) RETURNS BOOLEAN AS $$
BEGIN
  -- Delegate to unified RBAC guard if available; fallback to TRUE for testability
...
BEGIN
    ...
    END IF;
    -- Whitelist allowed table names to prevent SQL injection in dynamic SQL
    IF p_table_name NOT IN ('goods', 'persons', 'bill', 'ledger_entry', 'stock', 'event_store', 'bill_line') THEN
...
-- Compatibility wrapper for permission checks across RBAC variants
-- Returns FALSE if legacy RBAC tables are missing or in unknown state.
...
BEGIN
    BEGIN
        v_ok := sp_check_permission(p_user_id, p_permission_code);
    EXCEPTION WHEN OTHERS THEN
        -- If legacy RBAC is not available or disabled, deny by default for safety
        v_ok := FALSE;
    END;
    RETURN v_ok;
END;
