...
CREATE OR REPLACE VIEW v_projection_performance AS
SELECT 
    projection_name,
    event_type,
    COUNT(*) as total_runs,
...
-- Slow projection alert (threshold: >1000ms)
CREATE OR REPLACE FUNCTION projection_slow_alert(p_threshold_ms INT DEFAULT 1000)
RETURNS TABLE(projection_name TEXT, event_type TEXT, max_duration_ms INT) AS $$
BEGIN
    RETURN QUERY
