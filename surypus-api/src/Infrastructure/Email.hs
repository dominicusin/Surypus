...
data EmailConfig = EmailConfig
  { ecSmtpHost   :: !Text    -- ^ SMTP server hostname
  , ecSmtpPort   :: !Int     -- ^ SMTP server port (default 587)
  , ecSmtpUser   :: !Text    -- ^ SMTP authentication username
  , ecSmtpPass   :: !Text    -- ^ SMTP authentication password
  , ecFromAddr   :: !Text    -- ^ Sender email address
  , ecFromName   :: !Text    -- ^ Sender display name
  } deriving (Show, Eq)
