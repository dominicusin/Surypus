...
createPerson pool input = do
  result <- use pool $ Session.statement (piName input, piINN input, piKPP input, fromIntegral (piPersonType input), fromIntegral (piStatus input)) insertPersonStmt
  return $ case result of
    Left err -> QueryError (T.pack $ show err)
...
selectPersonsStmt = S.Statement sql encoder decoder True
  where
    sql = "SELECT id, name, inn, kpp, person_type, status FROM persons ORDER BY id LIMIT 50"
    encoder = E.noParams
...
selectPersonStmt = S.Statement sql encoder decoder True
  where
    sql = "SELECT id, name, inn, kpp, person_type, status FROM persons WHERE id = $1"
    encoder = ((\(pid) -> pid) >$< E.param (E.nonNullable E.int8))
    decoder = D.singleRow personDecoder

insertPersonStmt :: S.Statement (Text, Maybe Text, Maybe Text, Int32, Int32) Person
insertPersonStmt = S.Statement sql encoder decoder True
  where
    sql = "INSERT INTO persons (name, inn, kpp, person_type, status) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, inn, kpp, person_type, status"
    encoder =
      ((\(name, _, _, _, _) -> name) >$< E.param (E.nonNullable E.text))
        <> ((\(_, inn, _, _, _) -> inn) >$< E.param (E.nullable E.text))
        <> ((\(_, _, kpp, _, _) -> kpp) >$< E.param (E.nullable E.text))
        <> ((\(_, _, _, personType, _) -> fromIntegral personType) >$< E.param (E.nonNullable E.int4))
...
updatePersonStmt = S.Statement sql encoder decoder True
  where
    sql = "UPDATE persons SET name = $1, inn = $2, kpp = $3, person_type = $4, status = $5 WHERE id = $6 RETURNING id, name, inn, kpp, person_type, status"
    encoder =
      ((\(pi, _) -> piName pi) >$< E.param (E.nonNullable E.text))
        <> ((\(pi, _) -> piINN pi) >$< E.param (E.nullable E.text))
        <> ((\(pi, _) -> piKPP pi) >$< E.param (E.nullable E.text))
        <> ((\(pi, _) -> fromIntegral (piPersonType pi)) >$< E.param (E.nonNullable E.int4))
        <> ((\(pi, _) -> fromIntegral (piStatus pi)) >$< E.param (E.nonNullable E.int4))
...
searchPersonsStmt = S.Statement sql encoder decoder True
  where
    sql = "SELECT id, name, inn, kpp, person_type, status FROM persons WHERE name ILIKE $1 OR inn ILIKE $1 LIMIT 50"
    encoder = ((\(query) -> query) >$< E.param (E.nonNullable E.text))
    decoder = D.rowList personDecoder

personDecoder :: D.Row Person
personDecoder = Person <$> D.column (D.nonNullable D.int8) <*> D.column (D.nullable D.text) <*> D.column (D.nonNullable D.text) <*> D.column (D.nullable D.text) <*> D.column (D.nullable D.text) <*> D.column (D.nonNullable D.int2) <*> D.column (D.nonNullable D.int2)
