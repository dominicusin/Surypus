...
techCardFromRow (Single i, Single g, Single n, Single v, Single s, Single ca, Single ua, Single cb) =
    TechCard i g n v (fromIntegral s :: Int16) (fromMaybe epoch (parseUTCTime ca)) (fromMaybe epoch (parseUTCTime ua)) cb
...
workOrderFromRow (Single i, Single c, Single g, Single t, Single qp, Single qr, Single s, Single sd, Single ed, Single p, Single n, Single ca, Single ua, Single cb) =
    WorkOrder i c g t qp qr (fromIntegral s :: Int16) (sd >>= parseDay) (ed >>= parseDay) p n (fromMaybe epoch (parseUTCTime ca)) (fromMaybe epoch (parseUTCTime ua)) cb
...
createTechCard pool input = do
    ...
    let params = [ PersistInt64 (tgGoodsId input), PersistText (tgName input), PersistText (tgVersion input)
                 , PersistInt64 (fromIntegral (tgStatus input)), PersistText (fromMaybe "" (tgCreatedBy input))
                 ]
...
updateTechCard pool tcId input = do
    ...
    let params = [ PersistText (tgName input), PersistText (tgVersion input)
                 , PersistInt64 (fromIntegral (tgStatus input)), PersistText (fromMaybe "" (tgCreatedBy input))
                 , PersistInt64 tcId
                 ]
    rows <- liftIO $ runSqlPool (rawSql sql params) pool
...
createWorkOrder pool input = do
    ...
    let params = [ PersistText (woCode input), PersistInt64 (woGoodsId input)
                 ...
                 , case woStartDate input of { Just d -> PersistDay d; Nothing -> PersistNull }
                 , case woEndDate input of { Just d -> PersistDay d; Nothing -> PersistNull }
