...
- **Windsurf:** Uses `Agent(subagent_type="gsd-executor", ...)` — blocks until complete, returns result
...
@/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/agent-contracts.md
@/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/context-budget.md
@/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/gates.md
</required_reading>
...
**Model resolution:** If `executor_model` is `"inherit"`, omit the `model=` parameter from all `Agent()` calls — do NOT pass `model="inherit"` to Agent. Omitting the `model=` parameter causes Windsurf to inherit the current orchestrator model automatically. Only set `model=` when `executor_model` is an explicit model name (e.g., `"claude-sonnet-4-6"`, `"claude-opus-4-7"`).
...
```
Codex maps subagents to `spawn_agent`, which has no direct Codex mapping for Windsurf's `isolation="worktree"` parameter. Failing closed prevents main-checkout edits while the workflow believes agents are isolated.
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/home/domini/src/My/Surypus/.windsurf/get-shit-done/workflows/execute-plan.md` **inline**
...
between a large tool_result and the next assistant turn (seen on Windsurf
...
3. **Spawn executor agents:**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     # Windsurf inherits the orchestrator model automatically.
     ...
     prompt="
       ...
       spawned by Windsurf's `isolation="worktree"` use the `worktree-agent-<id>`
       ...
       prompt; silent bypass violates project .windsurf/rules guidance (#2924).
       ...
       <execution_context>
       @/home/domini/src/My/Surypus/.windsurf/get-shit-done/workflows/execute-plan.md
       @/home/domini/src/My/Surypus/.windsurf/get-shit-done/templates/summary.md
       @/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/checkpoints.md
       @/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/tdd.md
       @/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/home/domini/src/My/Surypus/.windsurf/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
4. **Wait for all agents in wave to complete.**
   ...
   **This fallback applies automatically to all runtimes.** Windsurf's Agent() normally
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Cross-wave dependency deviation (supported execution mode):** When the orchestrator legitimately deviates from the standard wave model — for example, a phase with cross-wave plan dependencies that requires custom inter-worktree base-update merges (e.g., `merge: bring 09-01 + 09-02 into 09-03 base`) — the cleanup loop below is NOT automatically re-entered for those custom merges. The deviation path produces correct final history but bypasses this loop, leaving `worktree-agent-*` directories in place. Use the **cleanup-tail snippet** below to remove any residual worktrees after such a deviation.
   ...
   # `.claude/worktrees/agent-` (the namespace Windsurf's `isolation="worktree"`
...
Read and follow `/home/domini/src/My/Surypus/.windsurf/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
