...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@/home/domini/src/My/Surypus/.codebuddy/get-shit-done/references/skeleton-template.md`.
...
**If found:**
1. Read template: `/home/domini/src/My/Surypus/.codebuddy/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.codebuddy/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS_PATH=$(ls ./.codebuddy/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/home/domini/src/My/Surypus/.codebuddy/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@/home/domini/src/My/Surypus/.codebuddy/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @/home/domini/src/My/Surypus/.codebuddy/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "/home/domini/src/My/Surypus/.codebuddy/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
