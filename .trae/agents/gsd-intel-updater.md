...
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "get-shit-done-cc" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .trae/get-shit-done 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
counts by running Glob on the layout-resolved canonical locations above, not from memory or .trae/rules/.
