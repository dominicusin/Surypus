...
@/home/domini/src/My/Surypus/.hermes/get-shit-done/references/agent-contracts.md
@/home/domini/src/My/Surypus/.hermes/get-shit-done/references/context-budget.md
@/home/domini/src/My/Surypus/.hermes/get-shit-done/references/gates.md
</required_reading>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/home/domini/src/My/Surypus/.hermes/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/home/domini/src/My/Surypus/.hermes/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/home/domini/src/My/Surypus/.hermes/get-shit-done/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       @/home/domini/src/My/Surypus/.hermes/get-shit-done/references/checkpoints.md
       @/home/domini/src/My/Surypus/.hermes/get-shit-done/references/tdd.md
       @/home/domini/src/My/Surypus/.hermes/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/home/domini/src/My/Surypus/.hermes/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Cross-wave dependency deviation (supported execution mode):** When the orchestrator legitimately deviates from the standard wave model — for example, a phase with cross-wave plan dependencies that requires custom inter-worktree base-update merges (e.g., `merge: bring 09-01 + 09-02 into 09-03 base`) — the cleanup loop below is NOT automatically re-entered for those custom merges. The deviation path produces correct final history but bypasses this loop, leaving `worktree-agent-*` directories in place. Use the **cleanup-tail snippet** below to remove any residual worktrees after such a deviation.
   ...
   done < <(git worktree list --porcelain | grep "^worktree " | grep "\.hermes/worktrees/agent-" | sed 's/^worktree //')
   ```
   ...
   **Cleanup-tail snippet (use after any wave whose merges did not flow through the templated path above):**
   ...
   # Inclusion-based filter (#2774): match ONLY agent-spawned worktrees under
   # `.hermes/worktrees/agent-`. Do NOT use exclusion filters (grep -v "$(pwd)$") —
...
7. **Handle failures:**
   ...
   **Known Hermes Agent bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Hermes Agent runtime bug — not a GSD or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 5 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
Read and follow `/home/domini/src/My/Surypus/.hermes/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
